...
std::string HelpMessage(HelpMessageMode hmm)
{
    string strUsage = _("Options:") + "\n" +
        "  -?                     " + _("This help message") + "\n" +
        "  -conf=<file>           " + _("Specify configuration file (default: vaultress.conf)") + "\n" +
        "  -pid=<file>            " + _("Specify pid file (default: vaultressd.pid)") + "\n" +
...
{
    RenameThread("vaultress-loadblk");
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    //
    // vaultress: -mintxfee and -minrelaytxfee options of bitcoin disabled
    ...
    if (mapArgs.count("-checkpointkey")) // vaultress: checkpoint master priv key
    {
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Vaultress is probably already running."), strDataDir.c_str()));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    printf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    printf("Vaultress version %s (%s)\n", FormatFullVersion().c_str(), CLIENT_DATE.c_str());
    ...
    if (fDaemon)
        fprintf(stdout, "Vaultress server starting\n");
    ...
    if (nLoadWalletRet != DB_LOAD_OK)
    {
        ...
        else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
        {
            ...
            strErrors << _("Error loading wallet.dat: Wallet requires newer version of Vaultress") << "\n";
        else if (nLoadWalletRet == DB_NEED_REWRITE)
        {
            strErrors << _("Wallet needed to be rewritten: restart Vaultress to complete") << "\n";
