...
// Note: rpcmining:GetNetworkHashPS uses bnProofOfWorkLimit as value
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 23);
static CBigNum bnInitialHashTarget(~uint256(0) >> 28);
// Note: rpcmining:GetNetworkHashPS uses same timespan
static const int64 nTargetTimespan = 12 * 60 * 60;
static const int64 nTargetSpacingWorkMax = 12 * STAKE_TARGET_SPACING;

unsigned int nStakeMinAge = STAKE_MIN_AGE;
int nCoinbaseMaturity = COINBASE_MATURITY_VAULT;
...
const string strMessageMagic = "Vaultress Signed Message:\n";
...
void SyncWithWallets(const uint256 &hash, const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // vaultress: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
...
bool CTransaction::CheckTransaction(CValidationState &state) const
{
    ...
    BOOST_FOREACH(const CTxOut& txout, vout)
    {
        ...
        // vaultress: enforce minimum output amount
        // v0.5 protocol: zero amount allowed
        if ((!txout.IsEmpty()) && txout.nValue < MIN_TXOUT_AMOUNT &&
            !(IsProtocolV05A(nTime) && (txout.nValue == 0)))
                        ...
                        bool* pfMissingInputs)
{
    ...
    // vaultress: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
int64 GetProofOfWorkReward(unsigned int prevHeight, unsigned int nBits)
{
    if(prevHeight == 0)
        return COIN_PREMINE;
...
// vaultress: miner's coin stake is rewarded based on coin age spent (coin-days)
int64 GetProofOfStakeReward(int64 nCoinAge)
{
	int maxReduction = 3;
	int reduction = std::min(pindexBest->nHeight / YEARLY_BLOCKCOUNT, maxReduction);

	// creation amount per coin-year
    static int64 nRewardCoinYear = COIN_YEAR_REWARD - reduction * CENT;
	
    int64 nSubsidy = nCoinAge * 33 / (365 * 33 + 8) * nRewardCoinYear;
	nSubsidy = min(nSubsidy, MAX_POS_REWARD);
...
unsigned int ComputeMinWork(unsigned int nBase, int64 nTime, bool fProofOfStake)
{
    ...
    bnResult *= 2;
    CBigNum bnTargetLimit = (fProofOfStake) ? GetProofOfStakeLimit(0, nTime) : bnProofOfWorkLimit;
...
// vaultress: find last block index up to pindex
...
#endif
{
    ...
    // vaultress: target change every block
    // vaultress: retarget with exponential moving toward target spacing
    CBigNum bnNew;
	...
	CBigNum bnTargetLimit = (fProofOfStake) ? GetProofOfStakeLimit(pindexLast->nHeight, pindexLast->nTime) : bnProofOfWorkLimit;
...
bool CheckProofOfWork(uint256 hash, unsigned int nBits)
{
    ...
    if (bnTarget > bnProofOfWorkLimit)
        return error("CheckProofOfWork() : nBits above maximum work");
...
void static InvalidChainFound(CBlockIndex* pindexNew)
{
    ...
    // vaultress: should not enter safe mode for longer invalid chain
}
...
bool CTransaction::CheckInputs(CValidationState &state, CCoinsViewCache &inputs, bool fScriptChecks, unsigned int flags, std::vector<CScriptCheck> *pvChecks) const
{
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // vaultress: check transaction timestamp
        ...
        if (IsCoinStake())
        {
            // vaultress: coin stake tx earns reward instead of paying fee
            uint64 nCoinAge;
        ...
        else
        {
            ...
            // vaultress: enforce transaction fees for every block
            if (nTxFee < GetMinFee())
...
bool CBlock::DisconnectBlock(CValidationState &state, CBlockIndex *pindex, CCoinsViewCache &view, bool *pfClean)
{
        ...
        if (i > 0) { // not coinbases
            ...
            for (unsigned int j = tx.vin.size(); j-- > 0;) {
                ...
                if (undo.nHeight != 0) {
                    ...
                    coins.fCoinStake = undo.fCoinStake; // vaultress
                    coins.nTime = undo.nTime;           // vaultress
                } else {
                    if (coins.IsPruned())
    ...
    // vaultress: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CValidationState &state, CBlockIndex* pindex, CCoinsViewCache &view, bool fJustCheck)
{
    ...
    // Adding reward check here (moved from CheckBlock)
    if (IsProofOfWork() && vtx[0].GetValueOut() > (IsProofOfWork()? (GetProofOfWorkReward(pindex->pprev ? pindex->pprev->nHeight : -1, pindex->nBits) - vtx[0].GetMinFee() + MIN_TX_FEE) : 0))
        return state.DoS(50, error("ConnectBlock() : coinbase reward exceeded %s > %s", 
                   FormatMoney(vtx[0].GetValueOut()).c_str(),
                   FormatMoney(IsProofOfWork()? GetProofOfWorkReward(pindex->pprev ? pindex->pprev->nHeight : -1, pindex->nBits) : 0).c_str()));
    ...
    // vaultress: coinbase reward check relocated to CheckBlock()
    ...
    // vaultress: track money supply and mint amount info
    pindex->nMint = nValueOut - nValueIn + nFees;
    pindex->nMoneySupply = (pindex->pprev? pindex->pprev->nMoneySupply : 0) + nValueOut - nValueIn;

    // vaultress: fees are not collected by miners as in bitcoin
    // vaultress: fees are destroyed to compensate the entire network
    if (fDebug && GetBoolArg("-printcreation"))
...
// vaultress: total coin age spent in transaction, in the unit of coin-days.
...
// vaultress: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(CValidationState &state, const CDiskBlockPos &pos)
{
    ...
    // vaultress: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // vaultress: record proof-of-stake hash value
    ...
    // vaultress: compute stake modifier
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // vaultress: only the second transaction can be the optional coinstake
    for (unsigned int i = 2; i < vtx.size(); i++)
        if (vtx[i].IsCoinStake())
            return state.DoS(100, error("CheckBlock() : coinstake in wrong position"));

    // vaultress: coinbase output should be empty if proof-of-stake block
    ...
    // Updated to check when is not POW; pow check is in ConnectBlock
    if (!IsProofOfWork() && vtx[0].GetValueOut() > 0)
        return state.DoS(50, error("CheckBlock() : coinbase reward exceeded %s > 0", 
                   FormatMoney(vtx[0].GetValueOut()).c_str()));
    ...
    // vaultress: check block signature
...
bool CBlock::AcceptBlock(CValidationState &state, CDiskBlockPos *dbp)
{
    ...
    if (hash != hashGenesisBlock) {
        ...
        // Allow PoW when applicable
        if (IsProofOfWork() && LAST_POW_BLOCK > 0 && nHeight > LAST_POW_BLOCK)
        ...
        // vaultress: check that the block satisfies synchronized checkpoint
    ...
    // vaultress: check pending sync-checkpoint
...
bool ProcessBlock(CValidationState &state, CNode* pfrom, CBlock* pblock, CDiskBlockPos *dbp)
{
    ...
    // vaultress: check proof-of-stake
    ...
    // vaultress: verify hash target and signature of coinstake tx
    if (pblock->IsProofOfStake())
    {
    ...
    // vaultress: ask for pending sync-checkpoint if any
    ...
    if (pblock->hashPrevBlock != 0 && !mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom) {
            ...
            // vaultress: getblocks may not obtain the ancestor block rejected
    ...
    // vaultress: if responsible for sync-checkpoint send it
...
// vaultress: entropy bit for stake modifier if chosen by modifier
...
bool static LoadBlockIndexDB()
{
    ...
    BOOST_FOREACH(const PAIRTYPE(int, CBlockIndex*)& item, vSortedByHeight)
    {
        ...
        // vaultress: calculate stake modifier checksum
    ...
    // vaultress: load hashSyncCheckpoint
...
bool LoadBlockIndex()
{
    ...
    printf("%s Network: genesis=0x%s nBitsLimit=0x%08x nBitsInitial=0x%08x nStakeMinAge=%d nCoinbaseMaturity=%d nModifierInterval=%d\n",
           fTestNet? "Test" : "Vaultress", hashGenesisBlock.ToString().substr(0, 20).c_str(), bnProofOfWorkLimit.GetCompact(), bnInitialHashTarget.GetCompact(), nStakeMinAge, nCoinbaseMaturity, nModifierInterval);
...
bool InitBlockIndex() {
    ...
    fTxIndex = true; // vaultress: txindex is always enabled
    ...
    if (!fReindex) {
        // Genesis block
        const char* pszTimestamp = "XKCD.com 25-AUG-2018 Supreme Court Bracket Sweet 16 -----------------------";
        CTransaction txNew;
        txNew.nTime = 1535169580;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1535169600;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 7456821u;

        if (fTestNet)
        {
            block.nTime    = 1535169590;
            block.nNonce   = 662498;
        }
        ...
        printf("%s\n", block.hashMerkleRoot.ToString().c_str());
        assert(block.hashMerkleRoot == uint256("0x5d821201ed9f9b4b8f6c81bbbe82316984a342d631ce053c62dbebea6874a53c"));
        block.print();
        assert(hash == hashGenesisBlock);
        // vaultress: check genesis block
        {
            CValidationState state;
        ...
        // vaultress: initialize synchronized checkpoint
        if (!WriteSyncCheckpoint(hashGenesisBlock))
            return error("LoadBlockIndex() : failed to init sync checkpoint");
    }

    // vaultress: if checkpoint master key changed must reset sync-checkpoint
...
string GetWarnings(string strFor)
{
    ...
    // vaultress: wallet lock warning for minting
    if (strMintWarning != "")
    {
        nPriority = 0;
    ...
    // vaultress: if detected invalid checkpoint enter safe mode
    ...
    // Alerts
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CAlert)& item, mapAlerts)
        {
            ...
            if (alert.AppliesToMe() && alert.nPriority > nPriority)
            {
                ...
                if (nPriority > 1000)
                    strRPC = strStatusBar;  // vaultress: safe mode for high alert
            }
        }
    }
...
void static ProcessGetData(CNode* pfrom)
{
    ...
    while (it != pfrom->vRecvGetData.end()) {
        ...
        const CInv &inv = *it;
        {
            ...
            // vaultress: relay memory may contain blocks too
            bool found = false;
            ...
            if (inv.type == MSG_BLOCK || inv.type == MSG_FILTERED_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    if (inv.hash == pfrom->hashContinue)
                    {
                        ...
                        // vaultress: send latest proof-of-work block to allow the
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "version")
    {
        ...
        // vaultress: record my external IP reported by peer
        ...
        // vaultress: relay sync-checkpoint
        {
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // vaultress: tell downloading node about the latest block if it's
...
CBlockTemplate* CreateNewBlock(CReserveKey& reservekey, CWallet* pwallet, bool fProofOfStake)
{
    ...
    if (!fProofOfStake && LAST_POW_BLOCK > 0 && nBestHeight >= LAST_POW_BLOCK)
		return NULL;
    ...
    // vaultress: if coinstake available add coinstake tx
    ...
    int64 nFees = 0;
    {
        ...
        while (!vecPriority.empty())
        {
            ...
            // vaultress: simplify transaction fee - allow free = false
        ...
        if (pblock->IsProofOfWork())
            pblock->vtx[0].vout[0].nValue = GetProofOfWorkReward(pindexPrev->nHeight, pblock->nBits);
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (hash > hashTarget && pblock->IsProofOfWork())
        return error("VaultressMiner : proof-of-work not meeting target");

    //// debug print
    printf("VaultressMiner:\n");
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("VaultressMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("VaultressMiner : ProcessBlock, block not accepted");
    }
...
#endif
{
    ...
    RenameThread(fProofOfStake? "vaultress-stake-minter" : "vaultress-miner");
    ...
    try { ploop {
        ...
        if (fProofOfStake)
        {
            // vaultress: if proof-of-stake block found then process block
            if (pblock->IsProofOfStake())
            {
        ...
        printf("Running VaultressMiner with %" PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        if(!fProofOfStake) printf("VaultressMiner terminated\n");
        throw;
    }
}
...
// vaultress: stake minter
void MintStake(boost::thread_group& threadGroup, CWallet* pwallet)
{
    // vaultress: mint proof-of-stake blocks in the background
