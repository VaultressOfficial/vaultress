...
static const int COINBASE_MATURITY_VAULT = 180;
/** Threshold for nLockTime: below this value it is interpreted as block number, otherwise as UNIX timestamp. */
static const int STAKE_TARGET_SPACING = 2 * 60; // block spacing 
...
static const int64 COIN_PREMINE = 8880000000 * COIN;
static const int64 COIN_YEAR_REWARD = 5 * CENT;
static const int64 MAX_POS_REWARD = 25 * COIN;
static const int64 LAST_POW_BLOCK = 7200; // Block height of the last proof of work block
static const int DAILY_BLOCKCOUNT = 720;
static const int YEARLY_BLOCKCOUNT = 262800;
...
static const uint256 hashGenesisBlockOfficial("0x0000002785fefeac5eb51d077b421b1f720ce8577add12fd725c4878975449e6");
static const uint256 hashGenesisBlockTestNet("0x000020a45a4d4745acb6aabfc3031a5b7fbb1db2a897e6708b651752478213c8");
...
public:
    ...
    {
        // vaultress: the coin stake transaction is marked with the first output empty
    ...
    */
    ...
    bool GetCoinAge(CValidationState &state, CCoinsViewCache &view, uint64& nCoinAge) const;  // vaultress: get transaction coin age
...
public:
    ...
    bool fCoinStake;      // vaultress: if the outpoint was the last unspent: whether it belonged to a coinstake
    unsigned int nTime;   // vaultress: if the outpoint was the last unspent: its tx timestamp
...
public:
    ...
    // vaultress: whether transaction is a coinstake
    bool fCoinStake;

    // vaultress: transaction timestamp
    unsigned int nTime;
    ...
    bool IsCoinStake() const { // vaultress: coinstake
        return fCoinStake;
    }
    ...
    bool Spend(const COutPoint &out, CTxInUndo &undo) {
        ...
        if (vout.size() == 0) {
            ...
            undo.fCoinStake = fCoinStake;  // vaultress
            undo.nTime = nTime;            // vaultress
        }
        return true;
    }
...
public:
    ...
    // vaultress: block signature - signed by coin base txout[0]'s owner
    ...
    // vaultress: two types of block: proof-of-work or proof-of-stake
    ...
    // vaultress: get max transaction timestamp
    ...
    bool GetCoinAge(uint64& nCoinAge) const; // vaultress: calculate total coin age spent in block
    bool SignBlock(const CKeyStore& keystore);
    bool CheckBlockSignature() const;
    unsigned int GetStakeEntropyBit() const; // vaultress: entropy bit for stake modifier if chosen by modifier
};
...
public:
    ...
    // (memory only) Total amount of trust score (vaultress proof-of-stake difficulty) in the chain up to and including this block
    uint256 nChainTrust;
    ...
    // vaultress: money supply related block index fields
    int64 nMint;
    int64 nMoneySupply;

    // vaultress: proof-of-stake related block index fields
    unsigned int nFlags;  // vaultress: block index flags
    enum
    {
