...
{
    if(Height > pindexBest->nHeight) { return "0000063eab58a239382210b23eeb1ef48ce3008997ad047fb79f3fe345baf96e"; }
    if(Height < 0) { return "0000063eab58a239382210b23eeb1ef48ce3008997ad047fb79f3fe345baf96e"; }
    int desiredheight;
...
std::string getOutputs(std::string txid)
{
    ...
    for (unsigned int i = 0; i < tx.vout.size(); i++)
    {
        ...
        {
			std::string lol7 = CBitcoinAddress(address).ToString();
...
std::string getInputs(std::string txid)
{
    ...
    for (unsigned int i = 0; i < tx.vin.size(); i++)
    {
        ...
        {
			std::string lol6 = CBitcoinAddress(address).ToString();
...
{    
    ...
    if(block == false) {
        ...
        ui->valueBox->setText(QValue + " VAULT");
        ui->txID->setText(QID);
        ui->outputBox->setText(QOutputs);
        ui->inputBox->setText(QInputs);
        ui->feesBox->setText(QFees + " VAULT");
    }
}
