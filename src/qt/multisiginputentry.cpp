...
void MultisigInputEntry::on_transactionId_textChanged(const QString &transactionId)
{
    ...
    for(int i = 0; i < tx.vout.size(); i++)
    {
        ...
        if(ExtractDestination(script, addr))
        {
            ...
            ui->transactionOutput->addItem(idStr + QString(" - ") + addressStr + QString(" - ") + amountStr + QString(" VAULT"));
        }
        else
            ui->transactionOutput->addItem(idStr + QString(" - ") + amountStr + QString(" VAULT"));
    }
}
