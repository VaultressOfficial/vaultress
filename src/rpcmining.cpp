...
Value GetNetworkHashPS(int lookup, int height) {
    ...
    if (lookup <= 0){
		...
		int nStakeTargetSpacing = STAKE_TARGET_SPACING;
    ...
    CBlockIndex *pb0 = pb;
	double totalDiff = pb0->GetBlockDifficulty();
    ...
    for (int i = 0; i < lookup; i++) {
		...
		totalDiff = totalDiff + pb0->GetBlockTime();
    }
	...
	double predictedBlockCount = (maxTime - minTime) / STAKE_TARGET_SPACING;
	double avgDiff = totalDiff / lookup;
	// using bnProofOfWorkLimit value
	double tmp = avgDiff * (2^20) / (lookup / predictedBlockCount);
	
    //printf("pb %f\n", predictedBlockCount);
    //printf("ad %f\n", avgDiff);
    //printf("tmp %f\n", tmp);

    return (boost::int64_t)(tmp);
}

Value getnetworkhashps(const Array& params, bool fHelp)
{
	// params[0] blocks default vaule is target number of blocks per day for the coin
    if (fHelp || params.size() > 2)
        throw runtime_error(
            "getnetworkhashps [blocks] [height]\n"
            "Returns the estimated network hashes per second based on the last 960 blocks.\n"
            "Pass in [blocks] to override # of blocks, -1 specifies since last difficulty change.\n"
            "Pass in [height] to estimate the network speed at the time when a certain block was found.");

    return GetNetworkHashPS(params.size() > 0 ? params[0].get_int() : 960, params.size() > 1 ? params[1].get_int() : -1);
}

// vaultress: get network Gh/s estimate
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Vaultress is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Vaultress is downloading blocks...");

    if (LAST_POW_BLOCK > 0 && pindexBest->nHeight >= LAST_POW_BLOCK)
