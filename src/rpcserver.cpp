...
// vaultress: remove transaction from mempool
...
// vaultress: reserve balance from being staked for network protection
...
// vaultress: check wallet integrity
...
Value stop(const Array& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
            "stop\n"
            "Stop Vaultress server.");
    // Shutdown will take long enough that the response should get back
    StartShutdown();
    return "Vaultress server stopping";
}
...
void StartRPCThreads()
{
        ...
        (mapArgs["-rpcuser"] == mapArgs["-rpcpassword"]))
    {
        ...
        string strWhatAmI = "To use vaultressd";
