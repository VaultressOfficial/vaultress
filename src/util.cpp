...
#else
    const char* pszModule = "vaultress";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Vaultress
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Vaultress
    // Mac: ~/Library/Application Support/Vaultress
    // Unix: ~/.vaultress
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Vaultress";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "vaultress.conf"));
    if (!pathConfigFile.is_complete()) pathConfigFile = GetDataDir(false) / pathConfigFile;

    // Load old config file if present
    if (mapArgs.count("-conf") == 0 && !boost::filesystem::exists(pathConfigFile)) {
        boost::filesystem::path pathOldConfigFile = GetDataDir(false) / "vaultress.conf";
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "vaultressd.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong Vaultress will not work properly.");
...
std::string FormatSubVersion(const std::string& name, int nClientVersion, const std::vector<std::string>& comments)
{
    ...
    ss << "/";
    ss << "Vaultress:" << FormatVersion(VAULTRESS_VERSION);
    ss << "(" << CLIENT_BUILD << ")/";
    return ss.str();
}
