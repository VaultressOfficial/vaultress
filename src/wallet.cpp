...
bool doSplitStake = false;
static unsigned int nStakeSplitAge = (60 * 60 * 24 * 5);
int64 nCombineThreshold = 10000;
...
// vaultress: optional setting to unlock wallet for block minting only;
    ...
    // restored from backup or the user making copies of wallet.dat.
    {
        ...
        BOOST_FOREACH(const CTxIn& txin, tx.vin)
        {
            ...
            if (mi != mapWallet.end())
            {
                ...
                {
                    printf("WalletUpdateSpent found spent coin %sacu %s\n", FormatMoney(wtx.GetCredit()).c_str(), wtx.GetHash().ToString().c_str());
...
int64 CWalletTx::GetTxTime() const
{
    ...
    //vaultress: we still have the timestamp, so use it to avoid confusion
    return nTime;
}
    ...
    while (fRepeat)
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CWalletTx)& item, mapWallet)
        {
            ...
            if (fFound || wtx.GetDepthInMainChain() > 0)
            {
                ...
                if (fUpdated)
                {
                    printf("ReacceptWalletTransactions found spent coin %sacu %s\n", FormatMoney(wtx.GetCredit()).c_str(), wtx.GetHash().ToString().c_str());
                    wtx.MarkDirty();
                    wtx.WriteToDisk();
                }
            }
...
// vaultress: total coins staked (non-spendable until maturity)
...

    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            if (pcoin->nTime > nSpendTime)
                continue;  // vaultress: timestamp must not exceed spend time
...

    {
        LOCK2(cs_main, cs_wallet);
        {
            ...
            ploop
            {
                ...
                // vaultress: sub-cent change is moved to fee
                ...
                if (nChange > 0)
                {
                    ...
                    else if (!GetBoolArg("-avatar")) // vaultress: not avatar mode; nu: avatar mode enabled by default only on Share wallet to avoid change being sent to hidden address
                    {
...
// vaultress: create coin stake transaction
    ...
    BOOST_FOREACH(PAIRTYPE(const CWalletTx*, unsigned int) pcoin, setCoins)
    {
        ...
        for (unsigned int n=0; n<min(nSearchInterval,(int64)nMaxStakeSearchInterval) && !fKernelFound; n++)
        {
            ...
            if (CheckStakeKernelHash(nBits, header, postx.nTxOffset + sizeof(CBlockHeader), *pcoin.first, prevoutStake, txNew.nTime - n, hashProofOfStake))
            {
                ...
                if (doSplitStake && header.GetBlockTime() + nStakeSplitAge > txNew.nTime)
...
// vaultress: check 'spent' consistency between wallet and txindex
// vaultress: fix wallet spent state according to txindex
void CWallet::FixSpentCoins(int& nMismatchFound, int64& nBalanceInQuestion, bool fCheckOnly)
{
    ...
    BOOST_FOREACH(CWalletTx* pcoin, vCoins)
    {
        ...
        for (unsigned int n=0; n < pcoin->vout.size(); n++)
        {
            ...
            {
                printf("FixSpentCoins found lost coin %sacu %s[%d], %s\n",
            ...
            {
                printf("FixSpentCoins found spent coin %sacu %s[%d], %s\n",
                    FormatMoney(pcoin->vout[n].nValue).c_str(), pcoin->GetHash().ToString().c_str(), n, fCheckOnly? "repair not attempted" : "repairing");
                nMismatchFound++;
                nBalanceInQuestion += pcoin->vout[n].nValue;
                if (!fCheckOnly)
...
// vaultress: disable transaction (only for coinstake)
